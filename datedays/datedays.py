...
class datedays():
    ...
    def getcurrent_monthsurplusday(self, current_date=None):
        '''
        获取指定月的剩余天数，
    ...
    def getnextmonth_surplusday(self, current_date=None, next_months=1):
        '''
        返回下个月日期列表 (自动跨年)
    ...
    def getmore(self, number=3):
        ...
        :return: list
        '''
        days_list = self.getcurrent_monthsurplusday()
        for i in range(1, number + 1):
            days_list += self.getnextmonth_surplusday(next_months=i)
        return days_list


if __name__ == '__main__':
    print(datedays().getcurrent_monthsurplusday('2023-08-05'))
    print(datedays().getnextmonth_surplusday())
    print(datedays().getmore())
